...
-->
 ...
 <xsl:template match="TEI">
  ...
  <xsl:value-of select="e:reportOn(count(teiHeader/fileDesc/sourceDesc//bibl[@type and not(@type = 'digitalSource' or @type = 'printSource' or @type = 'firstEdition')]), 'wrongly typed bibls corrected')"/>
 ...
 <xsl:template match="titleStmt/author">
  ...
  <xsl:choose>
   <xsl:when test="idno[@type = 'viaf'] and idno[@type = 'wiki']">
    <xsl:element name="author" xmlns="http://www.tei-c.org/ns/1.0">
     <xsl:attribute name="ref">
      <xsl:variable name="viafRef">
       <xsl:choose>
        <xsl:when test="starts-with(normalize-space(idno[@type = 'viaf']), 'http')">
         <xsl:value-of select="normalize-space(idno[@type = 'viaf'])"/>
        </xsl:when>
        <xsl:otherwise>
         <xsl:value-of select="concat('viaf:',normalize-space(idno[@type = 'viaf']))"/>       
        </xsl:otherwise>
       </xsl:choose>
      </xsl:variable><xsl:value-of select="
        concat($viafRef, ' ',
        normalize-space(idno[@type = 'wiki']))"/>
     </xsl:attribute>
  ...
  <xsl:attribute name="type">
   <xsl:choose>
    ...
    <xsl:when test=". eq 'print_source'">
     <xsl:if test="$verbose">
      <xsl:message>Wrongly typed bibl : changed 'print_source' to 'printSource'</xsl:message>
     </xsl:if>
     <xsl:text>printSource</xsl:text>
    </xsl:when>
...
<xsl:value-of select="p"/>
 ...
 <xsl:template match="revisionDesc">
  <xsl:copy>
   <change xmlns="http://www.tei-c.org/ns/1.0" when="{$today}">ran oldCheckUp to update</change>
   <xsl:apply-templates/>
  </xsl:copy>
 </xsl:template>
  ...
  <xsl:choose>
   ...
   <xsl:when test="starts-with(@target, '#')">
    ...
    <xsl:choose>
     ...
     <xsl:otherwise>
      <xsl:message>ERROR: Cannot find note with id <xsl:value-of select="$noteId"/></xsl:message>
     </xsl:otherwise>
    </xsl:choose>
    <xsl:copy-of select="."/>
   </xsl:when>
